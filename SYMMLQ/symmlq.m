function [x,flag,relres,iter,resvec,resveccg,errvec,errveccg] = ...
    symmlq(A,b,tol,maxit,eigest,M1,M2,x0,varargin)
% Contains additional 2-norm forward error estimates for SYMMLQ iterates
%   and CG-transfer point iterates. Completely untested.
...
%
%   X = SYMMLQ(A,B,TOL,MAXIT,LAMEST) specifies an underestimate of the
%   smallest eigenvalue of A for 2-norm upper-bound error estimates.
%
%   X = SYMMLQ(A,B,TOL,MAXIT,LAMEST,M) and X = SYMMLQ(A,B,TOL,MAXIT,M1,M2) use the
...
%
%   X = SYMMLQ(A,B,TOL,MAXIT,LAMEST,M1,M2,X0) specifies the initial guess. If X0
...
%   iteration.
%
%   [X,FLAG,RELRES,ITER,RESVEC,RESVECCG,ERRVEC] = SYMMLQ(A,B,...) also
%   returns a vector of upper bound estimates of the 2-norm forward error
%   NORM(X*-X). Requires eigest > 0.
%
%   [X,FLAG,RELRES,ITER,RESVEC,RESVECCG,ERRVEC,ERRVECCG] = SYMMLQ(A,B,...)
%   also returns a vector of upper bound estimates of the 2-norm forward
%   error of the CG transfer points NORM(X*-XCG). Requires eigest > 0.
%
%   Example:
...
if (n2b == 0)                      % if    rhs vector is all zeros
    ...
    if nargout >= 7, errvec = 0; end; % errvec(1) = norm(x*-x) = norm(0)
    if nargout >= 8, errveccg = 0; end; % errveccg(1) = norm(x*-xcg) = norm(0)
    if (nargout < 2)
...
if (normr <= tolb)                 % Initial guess is a good enough solution
    ...
    if nargout >= 7, errvec = 0; end; % Garbage value
    if nargout >= 8, errveccg = 0; end; % Garbage value
    if (nargout < 2)
...
if nargout >= 7
    errvec = zeros(maxit+2,1);         % Preallocate vector for error estimates
end
if nargout >= 8
    errveccg = zeros(maxit+2,1);         % Preallocate vector for CG error estimates
end
...
if (nargout >= 7 && eigest ~= 0)
    omega = eigest + beta*beta/(alpha-eigest);
    
    sigmawbar = beta;
    rhow = norm([(alpha-eigest) beta]);
    cw = (alpha-eigest)/rhow;
    sw = beta/rhow;
end
...
for ii = 1 : nits
    ...
    if (nargout >= 7 && eigest ~= 0)
        rhowbar = sw*sigmawbar - cw*(alpha-eigest);
    end    
    ...
    if (nargout >= 7 && eigest ~= 0)
        omega = eigest - cw*beta*beta/rhowbar;
        
        %tauw = sw*beta;
        sigmawbar = -cw*beta;
        rhow = norm([rhowbar beta]);
        cw = rhowbar/rhow;
        sw = beta/rhow;
    end
    ...
    if (nagout >= 8 && eigest ~= 0)
        errveccg(ii) = sqrt( zetanorm^2 - (epszdelz/gammabar)^2 );
    end    
    ...
    if (nargout >= 7 && eigest ~= 0)
        psi = cs*deltabar + sn*omega;
        omegabar = sn*deltabar - cs*omega;

        zetanorm = -(psi*zeta - epsilonzeta)/omegabar;
        errvec(ii) = zetanorm;
    end   
...
if ((flag <= 1) || (flag == 3))
    ...
    errveccg = errveccg(1:ii+1);
else
    resvec = resvec(1:ii);
    resveccg = resveccg(1:ii+1);
    errvec = errvec(1:ii);
    errveccg = errveccg(1:ii);
end
